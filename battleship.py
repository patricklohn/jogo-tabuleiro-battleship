...
Jogo baseado em tutoriais de Al Sweigart no livro 'Making Games with Python
& Pygame'
http://inventwithpython.com/pygame/chapters/

O jogo requer Python 3 e o módulo pygame.
É um duelo de batalha naval: cada jogador tem seu próprio tabuleiro e tenta
afundar todos os navios do oponente com o menor número de tiros.
"""
...
def main():
    pygame.init()
    global DISPLAYSURF, FPSCLOCK, BASICFONT, BIGFONT, HELP_SURF, HELP_RECT, NEW_SURF, NEW_RECT, EXPLOSION_IMAGES
...
def run_game():
    ...
    Retorna (vencedor, número_de_tiros).
    """
    players = ['Jogador 1', 'Jogador 2']
    turn_queue = deque(players)
    action_stack = []  # pilha de histórico de jogadas
...
def draw_board(board, rev):
    ...
    for i in range(BOARDWIDTH+1):
        pygame.draw.line(DISPLAYSURF, DARKGRAY, (XMARGIN+MARKERSIZE+i*TILESIZE, YMARGIN+MARKERSIZE), (XMARGIN+MARKERSIZE+i*TILESIZE, WINDOWHEIGHT-YMARGIN))
    for i in range(BOARDHEIGHT+1):
        pygame.draw.line(DISPLAYSURF, DARKGRAY, (XMARGIN+MARKERSIZE, YMARGIN+MARKERSIZE+i*TILESIZE), (WINDOWWIDTH-(DISPLAYWIDTH+MARKERSIZE*2), YMARGIN+MARKERSIZE+i*TILESIZE))

def set_markers(board):
    xm=[0]*BOARDWIDTH; ym=[0]*BOARDHEIGHT
    for x in range(BOARDWIDTH):
        for y in range(BOARDHEIGHT):
...
def add_ships_to_board(board, ships):
    ...
    for sh in ships:
        ...
        placed=False
        while not placed:
            xs=random.randint(0,BOARDWIDTH-1); ys=random.randint(0,BOARDHEIGHT-1)
    ...
    for dx in (-1,0,1):
        for dy in (-1,0,1):
            ...
            if 0<=nx<BOARDWIDTH and 0<=ny<BOARDHEIGHT:
                if board[nx][ny] not in (ship,None): return True
    return False

def left_top_coords_tile(tx,ty):
    return (tx*TILESIZE+XMARGIN+MARKERSIZE, ty*TILESIZE+YMARGIN+MARKERSIZE)

def get_tile_at_pixel(x,y):
    for tx in range(BOARDWIDTH):
        for ty in range(BOARDHEIGHT):
            lx,ly=left_top_coords_tile(tx,ty)
            if pygame.Rect(lx,ly,TILESIZE,TILESIZE).collidepoint(x,y): return tx,ty
    return None,None
